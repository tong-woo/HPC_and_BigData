...
int main(int argc, char* argv[]) {
	...
	for(int j = 0; j < sizeof(n)/sizeof(int); j++){
		float* a = new float[n[j]];
		float* b = new float[n[j]];
		float* result = new float[n[j]];
		float* result_s = new float[n[j]];
		if (argc > 1) n[j] = atoi(argv[1]);
                std::cout << "Adding two vectors of " << n[j] << " integer elements." << std::endl;
    		...
    		vectorAddSeq(n[j], a, b, result_s);
    		vectorAddCuda(n[j], a, b, result);
